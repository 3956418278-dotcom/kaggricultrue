...
Economic planning has a daily operating cadence. At the first observation of each day, `Plan` describes economic intent, mandatory owned-asset obligations, production goals, required inputs, staffing constraints/proposal, and land decisions using the dated work already carried by `(C, T, L, A, Q, R)`. New production is limited by resource/capacity feasibility, not a fixed project-count quota. Its land intervals and work schedules retain their dates and quantities but leave positions open; existing assets retain their actual locations. A staffing proposal from aggregate work is not a binding worker allocation.

An execution-only `Realization` binds those commitments to locations and dated hiring capacity without replacing daily intent. The intraday decision core compares complete reachable remaining-day trajectories, including worker divisions, task order, pickup/use chains, carrying, sales, placement, and the marginal executable benefit of hiring near day start. `rules.py` remains the single executable rule owner, including the owned-farm transition used by search. Search stops at deterministic daily refresh or the step-718/719 cash boundary; unknown opponent trades and future random shop unlocks are not predicted.

The controller retains the selected trajectory while its owned-state predictions remain valid. Current-market order construction stays reactive; execution divergence or changed purchase feasibility triggers a remaining-day search, preserving fulfilled placements and the daily economic goals. At most one economic repair is allowed when an existing physical premise is materially invalidated. A speculative fresh spatial binding or a shrinking turn count must not silently rewrite daily production. The next day's post-refresh observation starts fresh economic and intraday plans after automatic inventory drop, hand removal, and farmer reset.
