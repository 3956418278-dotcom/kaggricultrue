...
The runtime direction is strictly `current real state -> Daily Plan -> intraday planner -> executable trajectory`. At day start, the economic planner uses the complete real farm state, including layout, land availability, structures, assets, inventories and time, to decide what should be accomplished today. `Plan` owns economic objectives, mandatory obligations, resource requirements and state-derived constraints in `(C, T, L, A, Q, R)`. It does not own staffing or routing proposals. Production is limited by estimated resource/capacity feasibility, not a fixed project-count quota.

Existing-asset locations and economically relevant terrain constraints remain in Plan. For new economically equivalent placements, explicit placement domains retain the available freedom; an empty existing pasture and a tile requiring construction work are not silently interchangeable. `ExecutionChoices` is a separate execution record, not a subclass or replacement of Plan. Staffing/hiring, worker division, pickup quantities and timing, material allocation, carrying, task order, routing, logistics and allowed placement choices belong jointly to intraday realization. The legacy candidate currently explores only a restricted subset; this is an implementation limitation, not the intended architecture.

Selling is outside Daily Plan and is owned by `market.py`. Expected revenue may inform economic valuation, but a revenue estimate does not prescribe a sale schedule. Reactive selling does not rewrite production or maintenance objectives. Sale-financing is a realization-control concern only when it materially changes achievable work; default Fibonacci hand costs alone do not justify treating every sample as financing-dependent.
