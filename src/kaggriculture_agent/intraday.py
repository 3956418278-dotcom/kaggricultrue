...
def return_modes(programme: Programme):
    ...
    for asset in programme.assets:
        ...
        for event in events:
            ...
            liquidation_return = (
                event.source in {
                    "FEED_SUPPLY", "EXIT_LIQUIDATION",
                    "TERMINAL_LIQUIDATION", "F_RELAY"}
                and event.action
                and event.action[0] in {
                    "HARVEST", "COLLECT_FERTILIZER"})
            if ((event.action and event.action[0] == "HARVEST" and
                 (same_day_sale or event.source in {
                     "FEED_SUPPLY", "EXIT_LIQUIDATION",
                     "TERMINAL_LIQUIDATION"})) or
                    liquidation_return or needs_shed):
                mode = "MIDDAY_RETURN"
                break
...
def _compile_worker(state: State, day: int, worker: int, bundles: list[_Bundle], start: Position,
    ...
    if day_end == rules.TERMINAL_ACTION_STEP and sum(inventory.values()) > 0:
        access = _nearest_access(current, state.board_size)
        for action in _move_path(current, access):
...
def _compile_day(state: State, day: int, bundles, workforce: int, resource_avail: ResourceAvailability):
    ...
    inner = sorted(
        (bundle for bundle in bundles if bundle.zone == "INNER"),
        key=lambda bundle: (
            0 if any(event.source in {"FEED_SUPPLY", "F_RELAY"}
                     for event in bundle.events) else 1,
            bundle.deadline,
            bundle.tile[1],
            bundle.tile[0],
        ),
    ...
    for worker, work in enumerate(assigned):
        terminal_inventory = (
            day * rules.TURNS_PER_DAY <= rules.TERMINAL_ACTION_STEP
            < (day + 1) * rules.TURNS_PER_DAY
            and day == state.day
            and worker < len(state.workers)
            and bool(state.workers[worker].inventory))
        if not work and not terminal_inventory:
            continue
...
def _compile(state: State, programme: Programme, bundles, workforce: int, drops):
    ...
    day_set = {bundle.day for bundle in bundles}
    if (state.day * rules.TURNS_PER_DAY <= rules.TERMINAL_ACTION_STEP
            < (state.day + 1) * rules.TURNS_PER_DAY
            and any(worker.inventory for worker in state.workers)):
        day_set.add(state.day)
    days = sorted(day_set)
    for day in days:
...
def solve_intraday(state: State, programme: Programme, *, complete_actions: bool = True) -> Programme:
    ...
    for asset in programme.assets:
        ...
        for ev in asset.service_schedule:
            if ev.kind == "COLLECT_F" and ev.source == "F_RELAY":
                optimistic_drops[ev.step]["FERTILIZER"] += 1
    ...
    for workforce in range(minimum, maximum + 1):
        ...
        if complete_actions:
            ...
            days = sorted({bundle.day for bundle in bundles}) or [state.day]
            for day in days:
        ...
        hire_events = []
        quantity = max(0, workforce - len(state.workers))
        if quantity:
            hire_events.append(ProgrammeEvent(
                state.step, -100, f"hire:{state.day}", "HIRE",
                quantity=quantity))
