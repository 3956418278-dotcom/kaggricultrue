...
def reconstruct(observation: Any) -> State:
    ...
    return State(
        ...
        opp=OppState(opp_positions, opp_animals, opp_crops, opp_land, usable(opp_tiles), opp_tiles, int(get(opp_farm, "money")) if get(opp_farm, "money") is not None else None),
    )
