...
def realization_orders(state, worker_actions, purchases=(), hires=0, land=0, remaining_inputs=None):
    ...
    required = [["HIRE"] for _ in range(hires)] + [["BUY_LAND"] for _ in range(land)] + [list(p) for p in purchases]
    ...
    for item in sorted(rules.SELLABLE_PRODUCTS, key=lambda i: (-state.market_prices.get(i, 0), i)):
        reserve = max(0, remaining_inputs.get(item, 0) - after_units.carried_total(item))
