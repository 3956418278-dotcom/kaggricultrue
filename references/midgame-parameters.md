...
Animal MAINTAIN/EXIT and CARE no longer have replay-tuned thresholds or shadow
prices. They are event-driven next-cycle comparisons using official product and
Wheat prices plus exact incremental hire expenditure. F, shop bonuses, animal
count penalties, and fixed action/movement prices are excluded.

Animal placement first rejects every candidate whose economic advantage is not
positive. Remaining animal candidates use
`ranking_score = economic_advantage * (1 + locality_bonus)`, where
