...
- One model candidate is being developed under a **joint temporal constraint optimization** principle, not a sequence of deliberately weakened candidates or alternative settings of the legacy dispatcher. `references/intraday-model-design.md` records its formulation. Another model candidate must differ in design principle, not just restore omitted capabilities.
...
- Sixteen focused correctness tests have passed, including official trajectory parity, primitive-only projection, atomic seeds, shared pickup/feed, hiring availability, build/place ordering, fertilizer/water/harvest timing, decay, normal/terminal inventory handling, insertion-order overflow, land cost/availability, sale-entry truncation, retained execution and equivalent-asset witness labeling. Full-suite and final frozen-candidate reference verification remain pending after the latest implementation changes.
...
- Development diagnostics exposed expensive cold-start presolve, zero-stock resource dimensions and label-symmetry mismatch in starting hints. Eliminating provably irrelevant resources reduced an opening-day model from about 213,000 to 46,000 variables. Opening-day work can be fully realized, but mixed midgame work remains substantially below the reference, and the optimizer has not yet demonstrated meaningful gains beyond its starting witness at the small deterministic budget. Local total planning times are above the intended submission budget. This is **not a mature or reference-comparable planner**.
...
- The joint constraint encoding is expensive. Broad feasible domains alone have not yielded useful optimization within the action budget. Remaining limitations include material handoffs, physically achievable completion on dense farms, search effectiveness beyond initialization, and runtime. A favorable isolated day must not conceal these gaps.
...
The priority is a genuinely effective intraday model under the same-state/fixed-Plan reference benchmark—not further Dataset polishing. Finish the coherent model's correctness and reference checks, diagnose failures through actual reached states and search behavior, and improve or reject its design on that evidence. Do not promote a constructive initializer as successful joint optimization. A different candidate must use a genuinely different design principle. Keep held-out episodes untouched until a model and comparison protocol are frozen.
