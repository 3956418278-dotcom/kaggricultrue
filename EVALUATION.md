...
- Intraday development reports must distinguish the initial feasible witness, improvements produced by joint search, and an explicit planning failure. A fixed-witness or neighborhood optimum is not a global optimum. Count a physical effect at most once in semantic matching, reject placements outside Plan domains, and compare efficiency only alongside the achieved-goal set and resulting state. Retain exact source snapshots as well as hashes for uncommitted candidates. Report the recorded reference separately when using a controlled opponent-PASS background; flag material changes in its completion rather than silently treating a changed demonstration as ground truth.
- Keep code, schemas, tests and inspection commands maintained locally. Collect, validate and reconstruct large samples together in private CPU Kaggle runs; do not defer Plan reconstruction to a second collection-dependent job. Publish versioned private Kaggle Datasets with selection metadata, source hashes, per-episode checkpoints, exclusions and exact extractor identity. Resume only hash-compatible checkpoints; label runtime-bounded partial outputs explicitly and keep generated local copies ignored. Publication of completed cloud artifacts does not constitute a second reconstruction stage.
